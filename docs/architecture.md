...
Cards remain the default public discovery view. `view=table` selects a compact Material UI Table using the same dataset and server-side filter/sort/pagination semantics. `connection=signal` (or another supported type) filters across listing kinds. A JSONB containment predicate keeps counts and pages distinct even with multiple matching links. View switches retain paging; filter changes reset to page 1. Directory pages default to 25 records and offer 25, 50 or 100; the older 12, 24 and 48 values remain valid for shared URLs. Curated descriptors, connection-derived platform facets, proposal states and ownership assignment are documented in the [community entry design](community-entry-design.md) and [directory improvement checklist](directory-improvements.md).
