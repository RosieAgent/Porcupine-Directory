...
Security audits record account creation, role assignment, recovery, credential and email changes using internal IDs, not secrets. `/api/admin/audit` is administrator-only and paginated. Bookmarks never enter content revision history. Application routes do not update/delete audit rows, but this development database owner can: these logs are **not tamper-proof**. Before production, use separate migration/runtime database roles and restrict runtime audit permissions. Sensitive-content redaction and account deletion/export need an explicit retention workflow before public launch; do not store private information in entries or revision reasons.
...
- This loopback Compose setup is not a production deployment. Before launch: replace development DB credentials, least-privilege DB roles, TLS, persistent secrets, encrypted/restorable backups, abuse/reporting process, retention/redaction policy, restore drill, independent security review, and verify Hostinger supports this container stack.
