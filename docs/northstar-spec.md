...
Delivery update: typed multiple connections, connection filtering, table view, explicit proposals/seeking-organizer flags, missing-details badges, confirmation-first ordering, public contact fields, topic-icon presentation, saved tags, the controlled tag catalog and legacy compatibility are implemented. New creation work is moving to a branching wizard while the existing full edit form remains available to maintainers. Connection-derived platforms, curated descriptors and workflow states are being kept distinct. Private ownership assignment still requires recipient acceptance; member attestations, Signal ingestion and channel-content AI assistance remain future work. See the [community entry design](community-entry-design.md), [next-batch queue](tag-first-next-batch.md) and [delivery checklist](directory-improvements.md).
...
7. Maintain a shared tag catalog (name plus icon/emoji). Contributors select existing tags for their entries but cannot create or modify tag definitions. Creation answers may suggest a descriptor, but do not certify legal status or grant ownership.
...
View and connection filters live in the URL alongside existing query state; refresh, back/forward navigation and copying the URL reproduce the selection. Switching views preserves active filters, sort and paging. `/directory?connection=signal&view=table` opens the implemented Signal table. Material UI Table/TableContainer and the shared server-side search, sorting and pagination serve both views. Phones use two compact columns, with trust/access and saved controls beneath each entry name so connection links remain visible. Any overflow stays inside the table, not the whole page. Proposal context, card topic icons and the editor-managed tag catalog are implemented; the concise table omits topic columns.

### Tags — controlled catalog and icon presentation implemented

Current presentation maps legacy names to bundled Material UI icons (Housing → house; Learning → book), with a neutral tag fallback. Cards show all tags as compact icon links, wrapping rather than hiding overflow. Hover, keyboard focus and touch long-press expose their names; accessible link names always identify the topic. Details, search options and selected form topics pair names with icons. The database-backed catalog provides stable IDs, aliases, editor-only creation/rename/merge/retirement and contributor selection of existing definitions. No external images or tracking requests are used for icons.

Each shared tag has a stable identity, a human-readable name, and an editor-selected icon/emoji. Editors and administrators can create and maintain definitions. Ordinary users cannot create, rename or change tag icons, but can search and select existing tags when creating or editing their own entries. They can suggest a missing tag with a reason; authorized monitors/editors and administrators review the suggestion with an audited decision, and approval creates a normal catalog definition. Anonymous submissions likewise select from the existing catalog; neither public forms nor imports should silently create new catalog definitions.
...
### Tag-first taxonomy — implemented baseline, with legacy compatibility

New entries are entries described by controlled tags and connection-derived platform facets; legacy kind values remain for old routes, imports and history. Curated tags describe businesses, nonprofits, topics and other useful attributes. Creation answers may suggest controlled descriptors, while contributors can remove or change those suggestions before review. Only editors/administrators create or maintain definitions, and the server enforces the catalog for anonymous submissions and imports.
...
The public add form publishes an unconfirmed record by default and sends the contributor directly to the public entry after publication. A central submission policy can instead hold new entries for publication if abuse increases; no schema/UI redesign is needed. Ownership is private. Owners explicitly self-confirm saved content, editors can independently review it, and material changes clear both old badges. Anonymous entries have no submitter edit capability and cannot be automatically claimed. Rate limiting is implemented; broader bot protection, abuse reporting and operational moderation remain launch requirements.

The first creation flow is a branching wizard based on the [community entry design](community-entry-design.md). It asks for purpose, descriptor, proposed/existing state, organizer availability, public connection paths and participation instructions, then shows a public review before submission. The existing full form remains the editing surface until a later usability review. Business, nonprofit, organization, group and resource are overlapping descriptors; a Signal or website link is a connection point, not a claim of activity, trust, ownership or legal status.
